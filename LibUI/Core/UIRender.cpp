...
static bool bGdiplusEnable = false;

void CRenderEngine::GdiplusInit()
{
	if (!bGdiplusEnable)
	{
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						LibUI::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
...
void CRenderEngine::DrawColor(HDC hDC, const RECT& rc, DWORD color)
{
	...
	GdiplusInit();
	if (GetAValue(color) == 0xFF) color -= 0x01000000;
	Gdiplus::Graphics graphics(hDC);
	Gdiplus::SolidBrush brush(color);
	INT width = (INT)(rc.right - rc.left);
	INT height = (INT)(rc.bottom - rc.top);
	graphics.FillRectangle(&brush,(INT)rc.left,(INT)rc.top,width,height);
